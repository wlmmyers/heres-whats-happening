...
describe('PosterRequestSchema', () => {
  it('accepts a complete request', () => {
    const r = PosterRequestSchema.safeParse({
      userId: UID,
...
describe('PosterRequestSchema length bounds', () => {
  ...
  it('accepts a performer at the limit and rejects one over it', () => {
    ...
    expect(PosterRequestSchema.safeParse({ ...ok, performer: 'a'.repeat(201) }).success).toBe(
      false,
    );
  });
  ...
  it('counts code points, not UTF-16 code units', () => {
    ...
    expect(PosterRequestSchema.safeParse({ ...ok, performer: '🎸'.repeat(201) }).success).toBe(
      false,
    );
  });

  it('accepts 200 CJK characters, which are 600 bytes', () => {
    expect(PosterRequestSchema.safeParse({ ...ok, performer: '林'.repeat(200) }).success).toBe(
      true,
    );
  });
});
