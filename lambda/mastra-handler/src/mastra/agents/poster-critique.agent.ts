...
export const PosterCritiqueSchema = z.object({
  acceptable: z
    .boolean()
    .describe(
      'True only if this is a cool, legible concert poster showing the band photo, venue, and date.',
    ),
  critique: z
    .string()
    .describe(
      'If not acceptable, specific actionable fixes for the SVG author. If acceptable, a short note.',
    ),
});
