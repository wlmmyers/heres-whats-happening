...
export function substituteAndValidateSvg(
  svg: string,
  dataUri: string,
  ...
  if (!svg.includes(PLACEHOLDER)) {
    return {
      ok: false,
      svg,
      error: `SVG is missing the ${PLACEHOLDER} placeholder for the band image`,
    };
  }
  ...
  if (result !== true) {
    return {
      ok: false,
      svg: substituted,
      error: `SVG is not well-formed: ${result.err.msg} (line ${result.err.line})`,
    };
  }
...
export const svgParseTool = createTool({
  id: 'svg-parse',
  description:
    'Inject the band image data URI into the SVG placeholder and validate the SVG is well-formed.',
