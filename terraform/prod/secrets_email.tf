resource "aws_secretsmanager_secret" "email_llm_key" {
  name                    = "${var.app_name_prefix}/mastra-handler-llm-api-key"
  description             = "Anthropic API key for the mastra-handler Lambda. Seeded out-of-band."
  recovery_window_in_days = 7
  tags                    = { App = var.app_name_prefix }
}

# Placeholder version; the real key is written post-apply via:
#   aws secretsmanager put-secret-value --secret-id hwh/mastra-handler-llm-api-key --secret-string "<key>"
