 ...
 * field back to blank on the way in. It renders through a portal into Layout's
 * dialog root, since the calendar sidebar it is opened from sits inside
 * animated, transformed ancestors that would otherwise clip the backdrop and
 * trap the dialog in their stacking context.
 */
export default function AddManualEventDialog({ open, onClose, onCreated }: Props) {
  if (!open) return null;
  return <AddManualEventForm onClose={onClose} onCreated={onCreated} />;
}

function AddManualEventForm({ onClose, onCreated }: Omit<Props, 'open'>) {
  const titleId = useId();
  ...
  // portal and remount the form, losing whatever has been typed into it. The
  // fallback covers rendering the dialog outside a Layout, as the tests do.
  ...
  return createPortal(
    <div className={c.backdrop} onClick={onClose} data-testid="add-manual-event-backdrop">
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <p className={s.description}>
          It will appear in your Upcoming/Past lists, but won't be matched to your interests or
