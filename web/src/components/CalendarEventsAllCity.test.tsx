...
import { CalendarEventsAllCity } from './CalendarEventsAllCity';
import * as st from './SectionTitle.css';
import type { CalendarEvent } from '../api/calendar';

vi.mock('../api/calendar', () => ({ getCityCalendar: vi.fn() }));
...
// Later weeks are labelled with toLocaleDateString(), which follows the
...
// City events carry no match, so they arrive with an empty matched_because and
// a zero score.
...
function renderList() {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <MemoryRouter>
        <CalendarEventsAllCity gatePending={false} displayStyle="Full" />
      </MemoryRouter>
    </QueryClientProvider>,
  );
}
...
describe('CalendarEventsAllCity week section titles', () => {
  it('titles the section holding events from the current week "This week"', async () => {
    vi.mocked(getCityCalendar).mockResolvedValue({
      events: [event('c1', 'Wednesday Show', '2026-06-17T19:00:00-07:00')],
    });
  ...
  it('titles later sections with the date their week starts on', async () => {
    vi.mocked(getCityCalendar).mockResolvedValue({
      events: [
        event('c1', 'Next Week Show', '2026-06-23T19:00:00-07:00'),
        event('c2', 'Week After Show', '2026-07-02T19:00:00-07:00'),
      ],
    });
  ...
  it('renders each title in the list itself, ahead of the cards it introduces', async () => {
    vi.mocked(getCityCalendar).mockResolvedValue({
      events: [
        event('c1', 'Wednesday Show', '2026-06-17T19:00:00-07:00'),
        event('c2', 'Saturday Show', '2026-06-20T19:00:00-07:00'),
        event('c3', 'Next Week Show', '2026-06-23T19:00:00-07:00'),
      ],
    });
  ...
  it('gives a week one title however many events fall in it', async () => {
    vi.mocked(getCityCalendar).mockResolvedValue({
      events: [
        event('c1', 'Wednesday Show', '2026-06-17T19:00:00-07:00'),
        event('c2', 'Thursday Show', '2026-06-18T19:00:00-07:00'),
        event('c3', 'Saturday Show', '2026-06-20T19:00:00-07:00'),
      ],
    });
  ...
  // the list changes underneath — a page arriving, say.
